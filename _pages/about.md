---
permalink: /
title: "Erdemt Bao"
author_profile: true
redirect_from:
  - /about/
  - /about.html
---

## Biography {#biography}

Hi there! My name is **Erdemt Bao**, and I am currently a second-year master's student at the School of Mechanical Science and Engineering (MSE), Huazhong University of Science and Technology (HUST). Before my master's studies, I obtained my bachelor's degree from the School of Automation at Wuhan University of Technology in June 2024.

My research interests include **Robotics, Embodied Intelligence, and Autonomous Driving**. My current research mainly focuses on **robot skill learning**.

Please feel free to contact me via email at [baoerdemt366@gmail.com](mailto:baoerdemt366@gmail.com) if you are interested in my work or would like to collaborate with me.

I am currently seeking opportunities to apply for U.S. PhD programs for Fall 2027. Here is my [CV](/files/cv.pdf).

---

## News {#news}

* Recent activities and event photos (e.g., 2025 D-Robotics Developer, CICC Lumina, MIT Technology Review, graduation, SZU Open Day) — see the [Reports](#reports) section below for media coverage.

---

## Education {#education}

| | |
|---|---|
| ![HUST](/files/institute/hust.png){: width="230px" style="margin-left: 35px"} | **Huazhong University of Science and Technology (HUST)**  
| | [School of Mechanical Science & Engineering (MSE)](https://mse.hust.edu.cn/)  
| | Master's Student  
| | Oct. 2025 – Jun. 2029 (expected) |

| | |
|---|---|
| ![WHUT](/files/institute/whut.png){: width="220px" style="margin-left: 40px"} | **Wuhan University of Technology (WHUT)**  
| | [School of Automation (SA)](http://sa.whut.edu.cn/)  
| | B.Eng. in Computer Science and Technology (Outstanding Class)  
| | Sep. 2021 – Jul. 2025 |

---

## Research Experience {#research-experience}

| | |
|---|---|
| ![SCUT](/files/institute/scut.png){: width="250px" style="margin-left: 45px"} | **[MIAA Lab](https://miaa.cc/), South China University of Technology**  
| | **[2025 – 2026] Research Intern**, Embodied Intelligence |

---

## Publications {#publications}

{% for post in site.publications reversed %}
* **{{ post.title }}** — *{{ post.venue }}*, {{ post.date | date: "%Y" }}. [Details]({{ post.url }}){% if post.paperurl %} · [Paper]({{ post.paperurl }}){% endif %}
{% endfor %}

---

## Honors {#honors}

<div class="award-grid">
  <div class="award-card">
    <img src="/files/award/2025-neurips-mars-champion.jpg" alt="">
    <div class="award-text">NeurIPS 2025 MARS Challenge Manipulation Track Champion</div>
  </div>
  <div class="award-card">
    <img src="/files/award/2024-eccv_best_paper.png" alt="">
    <div class="award-text">ECCV 2024 MAAS Workshop<br>Best Paper Award</div>
  </div>
  <div class="award-card">
    <img src="/files/award/2025-agibotworld-challenge.jpg" alt="">
    <div class="award-text">IEEE IROS 2025 AgiBot World Challenge Third Runner-up</div>
  </div>
</div>

---

## Challenge Awards {#challenge-awards}

| | |
|:---:|:---|
| ![RoboTwin Real](/files/award/2025-Digital_Dynamics_REAL.png){: width="150px"} | **CVPR 2025 RoboTwin Dual-Arm Collaboration Challenge: Real-World Track** — ***First Prize***. With Yuanjie He, Yifei Liu. Supervised by [Prof. Jianbin Qin](https://szudseg.cn/people/). Team: SZU_Tencent |

| | |
|:---:|:---|
| ![RoboTwin Sim](/files/award/2025-Digital_Dynamics_SIM.png){: width="150px"} | **CVPR 2025 RoboTwin Dual-Arm Collaboration Challenge: Simulation Round 1** — ***Second Prize***. With Yuanjie He, Yifei Liu. Team: SZU_Tencent |

| | |
|:---:|:---|
| ![MCM/ICM](/files/award/2023-MCMICM.png){: width="150px"} | **2023 MCM/ICM** — ***Meritorious Winner***. With Weili Li, Jiaxuan Li. Supervised by [Dr. Jie Chen](https://csse.szu.edu.cn/pages/user/index?id=1130). Team: Shenzhen University |

---

## Engineering Projects {#engineering-projects}

**A Miniature Pipeline Inspection Robot Based on Structured-Light 3D Scanning** — *Erdemt Bao*, Jin Yang.

**A Robotic System for Disassembly and Recycling of Waste Printed Circuit Boards Toward Component Reuse** — *Erdemt Bao*, Jin Yang.

*Videos in `/files/work/` (e.g. `2022-Pipeline-robots.mp4`, `2023-Recycling-robots.mp4`).*

---

## Open-source Projects {#opensource-projects}

| | |
|:---:|:---|
| ![Awesome Robotics Skill Learning](/files/work/2026-Awesome-Robotics-Skill-Learning.png){: width="140px"} | **Awesome-Robotics-Skill-Learning** — *Founder & Project Lead & First Author*. [https://github.com/TianxingChen/RoboScholar](https://github.com/TianxingChen/RoboScholar) |

---

## Reports & Media {#reports}

* [05/2024] [WHUT Youth League Committee — Top 10 Outstanding Students & Teams](https://mp.weixin.qq.com/s/WEnskOukav5cQsm6CXvilA) — over 6,000 views.

* [04/2024] [WHUT Official — Profile: Erdemt Bao](https://mp.weixin.qq.com/s/gQ213_tOPVilCEQZjfAyZg) — over 8,000 views.

* [12/2023] [WHUT School of Automation — Shining Names: Erdemt Bao](http://218.197.103.36/xywh/xyrw/202503/t20250312_1321234.shtml) — official site.

---

## CV {#cv}

**Download PDF:** [CV](/files/cv.pdf)

Education, research experience, and publication lists are summarized above; see the [CV PDF](/files/cv.pdf) for the full version.
